...
export async function init() {
    ...
    // 1. Structure HTML / UI Modulaire avec Statistiques Dynamiques
    container.innerHTML = `
        <style>
            .prd-container { color: #f5f5f7; font-family: system-ui, -apple-system, sans-serif; background: #0d0d11; padding: 15px; border-radius: 12px; }
            ...
            /* DINETTE KPIs / STATISTIQUES DES PUBLICATIONS */
            ...
            .prd-thumb { width: 45px; height: 45px; border-radius: 6px; object-fit: cover; background: #20202c; border: 1px solid #282836; }
            ...
            .btn-act { padding: 6px 10px; border-radius: 4px; font-size: 11px; font-weight: 700; border: none; cursor: pointer; margin-right: 4px; transition: opacity 0.2s; }
        ...
        <div class="prd-container">
            <div class="prd-title">📦 Inventaire & Performance du Catalogue</div>

            <!-- CARTES ANALYTIQUES DES PRODUITS PUBLIÉS ET VENDUS -->
            <div class="kpi-grid">
            ...
            <div class="prd-header">
                <div class="prd-controls">
                    <input type="text" id="prd-search" class="prd-input" style="flex: 1; min-width: 200px;" placeholder="🔍 Rechercher un produit par nom ou ID...">
            ...
            <div class="prd-table-card">
                <table class="prd-table">
                    ...
                    <tbody id="prd-list-body">
                        <tr>
                            <td colspan="8" style="text-align: center; color: #ffcc00; padding: 20px;">Chargement du catalogue...</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    // Écoute en temps réel de la Realtime Database (`produits/`)
    ...
    onValue(productsRef, (snapshot) => {
        if (snapshot.exists()) {
            ...
            updateStatistics({});
            listBody.innerHTML = `<tr><td colspan="8" style="text-align: center; color: #a1a1aa; padding: 20px;">Aucun produit trouvé dans le catalogue.</td></tr>`;
        }
    });

    // Calcul des statistiques globales sur les publications et ventes
    function updateStatistics(data) {
        const prods = Object.values(data);
        const totalPublished = prods.length;
        ...
        prods.forEach(item => {
            // Cumul des ventes (supporte plusieurs clés courantes)
            totalSold += parseInt(item.nombreVentes || item.quantiteVendue || item.vendus || 0, 10);
            
            const stock = item.stock !== undefined ? parseInt(item.stock, 10) : null;
            const statut = item.statut || 'actif';

            if (statut === 'rupture' || (stock !== null && stock <= 0)) {
                outOfStockCount++;
            }
            if (statut === 'masque' || statut === 'inactif') {
                hiddenCount++;
            }
        });

        document.getElementById('stat-total-published').innerText = totalPublished;
    ...
    // Affichage filtré de la liste des produits
    function renderProducts() {
        const query = searchInput.value.toLowerCase().trim();
        const filterVal = statusFilter.value;
        let html = '';

        Object.keys(allProductsData).forEach((prodId) => {
            const item = allProductsData[prodId];
            const name = item.nom || item.titre || 'Sans nom';
            const statut = item.statut || 'actif';
            const stock = item.stock !== undefined ? item.stock : 'N/A';
            const vendus = item.nombreVentes || item.quantiteVendue || item.vendus || 0;
            const prix = item.prix || item.price || 0;
            const devise = item.devise || 'FCFA';
            const img = item.image || item.imageUrl || item.mediaUrl || 'https://via.placeholder.com/80';

            // Filtrage dynamique
            if (query && !name.toLowerCase().includes(query) && !prodId.toLowerCase().includes(query)) return;
            ...
            if (statut === 'masque' || statut === 'inactif') {
                ...
                badgeText = '🚫 Masqué';
            } else if (statut === 'rupture' || stock === 0) {
            ...
            html += `
                <tr>
                    <td><img src="${img}" class="prd-thumb" onerror="this.src='https://via.placeholder.com/80'" alt="Prod"></td>
                    <td>
                        <strong style="color: #fff; font-size: 14px;">${name}</strong>
                        <div style="font-size: 10px; color: #a1a1aa;">ID: ${prodId.slice(0, 10)}...</div>
                    </td>
                    ...
                    <td>
                        <button class="btn-act btn-toggle" onclick="toggleProductStatus('${prodId}', '${statut}')">
                            ${statut === 'actif' ? 'Masquer' : 'Activer'}
                        </button>
                        <button class="btn-act btn-del" onclick="deleteProduct('${prodId}')">🗑️</button>
                    </td>
                </tr>
            `;
        });

        listBody.innerHTML = html || `<tr><td colspan="8" style="text-align: center; color: #a1a1aa; padding: 20px;">Aucun produit ne correspond à votre recherche.</td></tr>`;
    }

    searchInput.addEventListener('input', renderProducts);
    statusFilter.addEventListener('change', renderProducts);

    // Basculer le statut d'un produit (Actif / Masqué)
    window.toggleProductStatus = async (prodId, currentStatus) => {
        const newStatus = currentStatus === 'actif' ? 'masque' : 'actif';
        try {
            await update(ref(db, `produits/${prodId}`), { statut: newStatus });
        } catch (err) {
    ...
    window.deleteProduct = async (prodId) => {
        if (confirm("⚠️ Voulez-vous vraiment supprimer définitivement ce produit du catalogue ?")) {
            try {
                await remove(ref(db, `produits/${prodId}`));
